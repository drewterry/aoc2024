...
func Solve(r io.Reader) any {
	...
	for _, print := range prints {
		...
		if !correct {
			corrected := correctPrint(rules, print)

			middle := (len(corrected) - 1) / 2
...
func correctPrint(rules map[int]map[int]struct{}, print []int) []int {
	corrected := print
	for i := 0; i < len(corrected); i++ {
		...
		rule, exists := rules[corrected[i]]
		if !exists {
			continue
		}

		for prevIndex, prevPage := range corrected[:i] {
			_, exists := rule[prevPage]
			if exists {
				corrected[prevIndex] = corrected[i]
				corrected[i] = prevPage
				i = 0
				break
			}
		}
